...
class MainWindow(QTabWidget):
    ...
    ***界面初始化***
    ...
    def initUI(self):
        # 图片检测子界面
        font_title = QFont('楷体', 25)
        font_main = QFont('楷体', 20)
        # 图片识别界面, 两个按钮，上传图片和显示结果

        img_detection_widget = QWidget()
        img_detection_widget.setStyleSheet("background-color:#ecf0f1;")
        #                                    "font-size:20px")

        # img_detection_widget.setStyleSheet("QPushButton{font-size:15px}")
        ...
        up_img_button.setStyleSheet("QPushButton{color:black}"
                                    "QPushButton:hover{background-color: rgb(190,190,145);}"
                                    "QPushButton{background-color:rgb(110,210,110)}"
                                    "QPushButton{border:2px}"
                                    "QPushButton{border-radius:5px}"
                                    "QPushButton{padding:20px 20px}"
                                    "QPushButton{margin:20px 20px}")
        ...
        self.webcam_detection_btn.setStyleSheet("QPushButton{color:black}"
                                                "QPushButton:hover{background-color: rgb(190,190,145);}"
                                                "QPushButton{background-color:rgb(110,210,110)}"
                                                "QPushButton{border:2px}"
                                                "QPushButton{border-radius:5px}"
        ...
        self.mp4_detection_btn.setStyleSheet("QPushButton{color:black}"
                                             "QPushButton:hover{background-color: rgb(190,190,145);}"
                                             "QPushButton{background-color:rgb(110,210,110)}"
                                             "QPushButton{border:2px}"
                                             "QPushButton{border-radius:5px}")
                                             # "QPushButton{padding:20px 20px}"
                                             # "QPushButton{margin:20px 20px}")
        self.vid_stop_btn.setStyleSheet("QPushButton{color:black}"
                                        "QPushButton:hover{background-color: rgb(190,190,145);}"
                                        "QPushButton{background-color:rgb(110,210,110)}"
                                        "QPushButton{border:2px}"
                                        "QPushButton{border-radius:5px}")
    ...
    ***上传图片***
    ...
    def upload_img(self):
        ...
        if fileName:
            ...
            save_path = osp.join("images/tmp", "test." + suffix)
            shutil.copy(fileName, save_path)
            # 应该调整一下图片的大小，然后统一防在一起
            im0 = cv2.imread(save_path)
            resize_scale = self.output_size / im0.shape[0]
            im0 = cv2.resize(im0, (0, 0), fx=resize_scale, fy=resize_scale)
            cv2.imwrite("images/tmp/test.jpg", im0)
            # self.right_img.setPixmap(QPixmap("images/tmp/single_result.jpg"))
            self.img2predict = fileName
            self.left_img.setPixmap(QPixmap("images/tmp/test.jpg"))
            self.right_img.setPixmap(QPixmap("images/tmp/test.jpg"))
        ...
        if source == "":
            ...
            for path, im, im0s, vid_cap, s in dataset:
                ...
                for i, det in enumerate(pred):  # per image
                    ...
                    s=s[-11:-2]
                    im0 = cv2.putText(im0, 'person:'+s, (50, 50), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 200), 3)
                    ...
                    # test='person:'+n
                    # print(test)
                    # im0=cv2.putText(im0, 'person:', (10,10),15,(0,0,255))
                    cv2.imwrite("images/tmp/test.jpg", im0)
    ...
    def detect_vid(self):
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                s = s[-11:-2]
                im0 = cv2.putText(im0, 'person:' + s, (100, 100), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 255, 200), 6)
                frame = im0
                resize_scale = output_size / frame.shape[0]
                frame_resized = cv2.resize(frame, (0, 0), fx=resize_scale, fy=resize_scale)
                cv2.imwrite("images/tmp/test.jpg", frame_resized)
                self.vid_img.setPixmap(QPixmap("images/tmp/test.jpg"))
